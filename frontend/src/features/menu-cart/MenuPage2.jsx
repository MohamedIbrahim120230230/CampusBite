...
function ItemCard({ item, cartQty, onAdd, onRemove, locked }) {
  ...
  // Use manual image_url if set, otherwise auto-fetch from Unsplash by item name
  const autoUrl = `https://source.unsplash.com/400x300/?${encodeURIComponent(item.name + " food")}`;
  const imgSrc  = (item.image_url && item.image_url.trim()) ? item.image_url : autoUrl;
    ...
    <div className={`mp-item ${outOfStock ? "mp-item--oos" : ""}`}>
      ...
      <div className="mp-item-img-wrap">
        {!imgError ? (
          ...
          <img
            src={`https://source.unsplash.com/400x300/?${encodeURIComponent(item.category + " food")}`}
            alt={item.name}
            className="mp-item-img"
          />
        )}
