...
const CAT_PLACEHOLDER = {
  meals:      { emoji: "🍽️", grad: "linear-gradient(135deg,#1a2a1a,#0f1f0f)", accent: "#22c993" },
  beverages:  { emoji: "☕", grad: "linear-gradient(135deg,#1a1a2a,#0f0f1f)", accent: "#3b9eda" },
  snacks:     { emoji: "🍪", grad: "linear-gradient(135deg,#2a1a0f,#1f150a)", accent: "#f6c90e" },
  default:    { emoji: "🍴", grad: "linear-gradient(135deg,#1a1825,#111825)", accent: "#6b7a90" },
};
...
export default function MenuPage() {
  ...
  const guardLocked = () => {
    if (cartLocked) { addToast("Cart is locked — checkout in progress.", "warn"); return true; }
    return false;
  };
  ...
  const addToCart = async (item) => {
    ...
    try {
      await apiFetch("/cart/add", { method: "POST", body: JSON.stringify({ item_id: item.id, qty: 1 }) });
    } catch (err) {
      if (err?.code === "CART_LOCKED") { setCartLocked(true); addToast("Cart is locked.", "warn"); }
    }
  };

  const removeFromCart = (id) => { if (guardLocked()) return; setCart(prev => prev.filter(c => c.id !== id)); };

  const updateQty = (id, qty) => {
    if (guardLocked()) return;
    if (qty < 1) { removeFromCart(id); return; }
    const item = items.find(i => i.id === id);
    if (item && qty > item.max_order_qty) { addToast(`Max order quantity for "${item.name}" is ${item.max_order_qty}.`, "warn"); return; }
  ...
  const applyVoucher = async () => {
    if (voucherApplied) { setVoucherError(VOUCHER_ERRORS.VOUCHER_STACK_REJECTED); return; }
    if (!voucher.trim()) return;
    setVoucherLoading(true); setVoucherError("");
    try {
      const data = await apiFetch("/cart/voucher", { method: "POST", body: JSON.stringify({ code: voucher.trim().toUpperCase() }) });
      const dtype  = data.discount_type ?? "flat";
      const dvalue = data.discount_value ?? data.discount ?? data.discount_egp ?? 0;
      const discountAmt = dtype === "percent" ? Math.min((dvalue / 100) * subtotal, subtotal) : Math.min(dvalue, subtotal);
      ...
      setDiscount(discountAmt); setVoucherApplied(true);
      setAppliedVoucherObj({ code: voucher.trim().toUpperCase(), discount_type: dtype, discount_value: dvalue });
    } catch (err) {
      const code = err?.code ?? "";
      setVoucherError(VOUCHER_ERRORS[code] ?? err?.message ?? "Invalid voucher code.");
    } finally { setVoucherLoading(false); }
  };

  const removeVoucher = () => {
    setVoucherApplied(false); setDiscount(0); setVoucher(""); setVoucherError(""); setAppliedVoucherObj(null);
    addToast("Voucher removed.", "success");
  };

  const handleCheckout = async () => {
    if (cart.length === 0) return;
    setCheckoutLoading(true); setLockWarnings([]);
    try {
      ...
      navigate("/order", {
        state: { cart, subtotal, discount: appliedDiscount, total: finalTotal, voucherCode: appliedVoucherObj?.code ?? null, lockedOrder: data.order ?? null },
      });
    } catch (err) {
      if (err?.code === "ITEM_OUT_OF_STOCK") { addToast(`${err.message}`, "error"); }
      else if (err?.code === "CART_LOCKED") { setCartLocked(true); addToast("Cart is already locked.", "warn"); }
      else { addToast(err?.message || "Checkout failed. Please try again.", "error"); }
      ...
      <div className="mp-page">
        ...
        <nav className="mp-nav">
          ...
          <div className="mp-nav-actions">
            <button className="mp-cart-btn" onClick={() => setCartOpen(v => !v)} aria-label={`Cart — ${cartCount} items`}>
        ...
        <div className={`mp-layout ${cartOpen ? "mp-layout--cart-open" : ""}`}>
          <main className="mp-main">
            ...
            <div className="mp-search-wrap">
              ...
              <input
                ref={searchRef} className="mp-search" type="text" placeholder="Search menu…"
                value={search} onChange={e => setSearch(e.target.value)}
                onKeyDown={e => e.key === "Enter" && fetchMenu()}
              />
              {search && <button className="mp-search-clear" onClick={() => { setSearch(""); fetchMenu(); }}><i className="bi bi-x" /></button>}
              <button className="mp-search-btn" onClick={fetchMenu}>Search</button>
            </div>

            <div className="mp-cats" role="tablist">
              {CATEGORIES.map(c => (
                <button key={c.value} role="tab" aria-selected={category === c.value}
              ...
              <div className="mp-warn-banner">
                ...
                <div>
                  ...
                  {lockWarnings.map((w, i) => (
                    <div key={i} className="mp-warn-item">{w.item}: {w.old_price} EGP → <strong>{w.new_price} EGP</strong></div>
                  ))}
                  <button className="mp-warn-confirm" onClick={confirmWarningsAndCheckout}>Confirm new prices &amp; proceed</button>
                </div>
              </div>
            )}

            {loading ? (
              <div className="mp-loading"><div className="mp-spinner" /><span>Loading menu…</span></div>
            ) : items.length === 0 ? (
          ...
          <aside className={`mp-cart ${cartOpen ? "mp-cart--open" : ""}`} aria-label="Shopping cart">
            <CartPanel
              cart={cart} subtotal={subtotal} finalTotal={finalTotal} appliedDiscount={appliedDiscount}
              voucher={voucher} setVoucher={setVoucher} voucherApplied={voucherApplied}
              voucherError={voucherError} voucherLoading={voucherLoading}
              onApplyVoucher={applyVoucher} onRemoveVoucher={removeVoucher}
              onUpdateQty={updateQty} onRemoveItem={removeFromCart}
              onCheckout={handleCheckout} checkoutLoading={checkoutLoading}
              cartLocked={cartLocked} onClose={() => setCartOpen(false)}
            />
          </aside>
        </div>
...
// Maps item name/category keywords to Foodish API food types
function getFoodishUrl(name, category) {
  const n = (name || "").toLowerCase();
  const c = (category || "").toLowerCase();
  if (n.includes("burger"))                          return "https://foodish-api.com/images/burger/burger1.jpg";
  if (n.includes("pizza"))                           return "https://foodish-api.com/images/pizza/pizza1.jpg";
  if (n.includes("pasta") || n.includes("spaghetti"))return "https://foodish-api.com/images/pasta/pasta1.jpg";
  if (n.includes("rice") || n.includes("biryani"))   return "https://foodish-api.com/images/biryani/biryani1.jpg";
  if (n.includes("chicken") || n.includes("grilled"))return "https://foodish-api.com/images/butter-chicken/butter-chicken1.jpg";
  if (n.includes("salad") || n.includes("wrap"))     return "https://foodish-api.com/images/samosa/samosa1.jpg";
  if (n.includes("sandwich") || n.includes("sub"))   return "https://foodish-api.com/images/burger/burger2.jpg";
  if (n.includes("cake") || n.includes("dessert") || n.includes("sweet")) return "https://foodish-api.com/images/dessert/dessert1.jpg";
  if (n.includes("coffee") || n.includes("tea") || n.includes("drink") || n.includes("water") || n.includes("juice") || c === "beverages") return "https://foodish-api.com/images/rice/rice1.jpg";
  if (c === "snacks")  return "https://foodish-api.com/images/samosa/samosa2.jpg";
  if (c === "meals")   return "https://foodish-api.com/images/butter-chicken/butter-chicken2.jpg";
  return "https://foodish-api.com/images/pizza/pizza2.jpg";
}
...
function ItemCard({ item, cartQty, onAdd, onRemove, locked }) {
  ...
  const [imgSrc, setImgSrc] = useState(
    (item.image_url && item.image_url.trim())
      ? item.image_url
      : getFoodishUrl(item.name, item.category)
  );
  const ph = CAT_PLACEHOLDER[item.category] || CAT_PLACEHOLDER.default;
      ...
      <div className="mp-item-body">
        ...
        <div className="mp-item-footer">
          ...
          {outOfStock || locked ? (
            ...
            <button className="mp-add-btn" onClick={onAdd}><i className="bi bi-plus-lg" /> Add</button>
          ) : (
            <div className="mp-qty-ctrl">
              <button onClick={onRemove} aria-label="Decrease quantity"><i className="bi bi-dash" /></button>
              <span>{cartQty}</span>
              <button onClick={onAdd} disabled={atCap} aria-label="Increase quantity" title={atCap ? `Max ${item.max_order_qty} per order` : "Add one more"}>
                <i className="bi bi-plus" />
              </button>
            </div>
          )}
        </div>
        {atCap && !outOfStock && <p className="mp-cap-msg"><i className="bi bi-info-circle me-1" />Max {item.max_order_qty} per order</p>}
      </div>
    </div>
  );
}
...
function CartPanel({
  ...
  onApplyVoucher, onRemoveVoucher, onUpdateQty, onRemoveItem,
    ...
    <div className="mp-cart-inner">
      <div className="mp-cart-hd">
        <h2 className="mp-cart-title">
          <i className="bi bi-bag-fill" />Your Cart
          {cartLocked && <span className="mp-cart-locked-tag"><i className="bi bi-lock-fill" /> Locked</span>}
        </h2>
        <button className="mp-cart-close" onClick={onClose} aria-label="Close cart"><i className="bi bi-x-lg" /></button>
      </div>
      ...
      {cart.length === 0 ? (
        ...
        <>
            ...
            <div className="mp-voucher">
              {voucherApplied ? (
                <div className="mp-voucher-applied">
                  ...
                  <button onClick={onRemoveVoucher} className="mp-voucher-remove" aria-label="Remove voucher"><i className="bi bi-x" /></button>
                </div>
              ) : (
                <>
                  <div className="mp-voucher-row">
                    <div className="mp-voucher-iw">
                      ...
                      <input
                        className="uc-input mp-voucher-input" type="text" placeholder="Voucher code"
                        value={voucher} onChange={e => { setVoucher(e.target.value.toUpperCase()); }}
                        onKeyDown={e => e.key === "Enter" && onApplyVoucher()} disabled={voucherLoading}
                      />
                    </div>
                    <button className="mp-voucher-btn" onClick={onApplyVoucher} disabled={voucherLoading || !voucher.trim()}>
                      {voucherLoading ? <span className="mp-spinner-sm" /> : "Apply"}
                    </button>
                  </div>
                  {voucherError && <p className="mp-voucher-err"><i className="bi bi-exclamation-circle me-1" />{voucherError}</p>}
                </>
              )}
            </div>
          )}
          ...
          <div className="mp-totals">
            <div className="mp-totals-row"><span>Subtotal</span><span>{subtotal.toFixed(2)} EGP</span></div>
            {appliedDiscount > 0 && (
          ...
          <button className="mp-checkout-btn" onClick={onCheckout} disabled={checkoutLoading || cartLocked || cart.length === 0}>
            {checkoutLoading ? <><span className="mp-spinner-sm" /> Processing…</> :
             cartLocked      ? <><i className="bi bi-lock-fill" /> Order Locked</> :
...
const MENU_CSS = `
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .mp-item { background:var(--uc-card); border:1px solid var(--uc-brd); border-radius:var(--uc-r);
    overflow:hidden; position:relative; transition:border-color .25s,transform .2s; display:flex; flex-direction:column; }
  ...
  .mp-item-img-glow { position:absolute; width:80px; height:80px; border-radius:50%; opacity:.18; filter:blur(24px); }

  /* ── Badges (now over image) ── */
  .mp-badge { position:absolute; top:10px; right:10px; font-size:10px; font-weight:700; padding:3px 8px; border-radius:100px; letter-spacing:.04em; z-index:2; backdrop-filter:blur(6px); }
  ...
  .mp-item-cat { font-size:10px; font-weight:600; letter-spacing:.08em; text-transform:uppercase; color:var(--uc-muted); margin-bottom:5px; display:block; }
