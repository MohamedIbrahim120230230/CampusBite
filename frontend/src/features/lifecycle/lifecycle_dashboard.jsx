...
//
// Order lifecycle management dashboard — integrated into the
// main Cafeteria OS router at /lifecycle.
//
// Role + actorId are injected by the parent router (no internal
// role-switcher) so the authenticated session context is used.
// ============================================================
...
function OrderCard({ order, role, actorId, onRefresh, onToast }) {
  ...
  // Normalise: the router stores lowercase roles; the lifecycle API expects uppercase.
  ...
  async function doAdvance() {
    const { ok, body } = await apiFetch(`/orders/${order.id}/status`, {
      method: "PATCH",
      headers: headers(normRole, actorId),
...
function OrdersTab({ role, actorId, toast }) {
  ...
  const load = useCallback(async () => {
    ...
    const results = await Promise.all(ids.map(id =>
      apiFetch(`/orders/${id}`, { headers: headers(normRole, actorId) })
    ));
        ...
        <div data-testid="orders-list">
          {Object.entries(grouped).map(([group, list]) => list.length > 0 && (
            <div key={group} style={{ marginBottom: 28 }}>
              ...
              {list.map(order => (
                <OrderCard key={order.id} order={order} role={normRole} actorId={actorId}
    ...
    <div>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "flex-end" }}>
          <div>
            <label style={{ color: "#6B7280", fontSize: 12, display: "block", marginBottom: 4 }}>Report Type</label>
            <select value={type} onChange={e => setType(e.target.value)}
              style={{
                background: "#1F2937", border: "1px solid #374151",
                borderRadius: 7, padding: "8px 12px", color: "#F9FAFB", fontSize: 13,
              }}>
          ...
          <div>
            <label style={{ color: "#6B7280", fontSize: 12, display: "block", marginBottom: 4 }}>From</label>
          ...
          <div>
            <label style={{ color: "#6B7280", fontSize: 12, display: "block", marginBottom: 4 }}>To</label>
        ...
        {dateError && <div style={{ color: "#F87171", fontSize: 12, marginTop: 8 }}>{dateError}</div>}
      </div>
      ...
      {asyncJob && (
        <div style={{
          background: "#111827", border: "1px solid #1F2937",
          borderRadius: 12, padding: 20, color: "#FCD34D",
        }}>
          ⏳ Report queued — job <code>{asyncJob.job_id}</code>. Check back shortly.
        </div>
      )}
...
function FlaggedTab({ role, actorId, toast }) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const { ok, body } = await apiFetch("/admin/flagged-orders", { headers: headers(normRole, actorId) });
    setLoading(false);
...
function ConfigTab({ role, actorId, toast }) {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      const { ok, body } = await apiFetch("/admin/config", { headers: headers(normRole, actorId) });
      setLoading(false);
...
function AuditTab({ role, actorId }) {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      const { ok, body } = await apiFetch("/admin/audit-log?limit=50", { headers: headers(normRole, actorId) });
      setLoading(false);
      if (ok) setLogs(body);
    })();
  }, []);
...
// ── LifecycleDashboard ────────────────────────────────────────────────────────
//
// Props injected by the router:
//   role    — lowercase string from localStorage user object ("admin" | "staff")
//   actorId — user id string from localStorage user object
//
// Both values are normalised to UPPERCASE internally before reaching the API.
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div>
          ...
          <span style={{ color: "#374151", marginLeft: 12, fontSize: 12 }}>
            lifecycle · {normRole.toLowerCase()}
          </span>
        </div>

        {/* Role badge — read-only, reflects authenticated session */}
        <div style={{ marginLeft: "auto", display: "flex", gap: 10, alignItems: "center" }}>
          <span style={{
            background: "#1F2937", border: "1px solid #374151",
            borderRadius: 7, padding: "6px 14px", color: "#F9FAFB", fontSize: 13,
          }}>
            {normRole}
          </span>
          <span style={{ color: "#6B7280", fontSize: 12 }}>
            {normRole === "ADMIN" ? "🔴 Admin" : normRole === "STAFF" ? "🟡 Staff" : "🟢 Student"}
          </span>
        </div>
      </div>
      ...
      <div style={{ padding: "28px", maxWidth: 900, margin: "0 auto" }}>
        {tab === "orders"  && <OrdersTab  role={normRole} actorId={actorId} toast={showToast} />}
        {tab === "reports" && <ReportsTab role={normRole} actorId={actorId} toast={showToast} />}
        {tab === "flagged" && <FlaggedTab role={normRole} actorId={actorId} toast={showToast} />}
        {tab === "config"  && <ConfigTab  role={normRole} actorId={actorId} toast={showToast} />}
